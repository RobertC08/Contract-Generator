...
- **Placeholder cu fallback (`|`):** poți folosi un singur tag, ex. `{studentFullName | guardianFullName}` sau `{studentAddress | guardianAddress}`. În link-ul de completare, aceste **două perechi** apar ca **câte un singur câmp** (eticheta = alias din `variableDefinitions` dacă e setat). Valoarea introdusă se salvează ca elev sau apărător după `hasGuardian` din `integrationMetadata`. La generarea DOCX, același tag folosește aceeași regulă (apărător prioritar dacă `hasGuardian: "true"`, altfel elev). Alte tag-uri cu `|` rămân câte un câmp per segment; coalesce generic = prima valoare nevidă stânga→dreapta.
