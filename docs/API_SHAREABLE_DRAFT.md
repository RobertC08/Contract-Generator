...
- **Placeholder cu fallback (`|`):** poți folosi un singur tag, ex. `{studentFullName | guardianFullName}` sau `{studentAddress | guardianAddress}`. Nu apare câmp separat în formular; completezi variabilele normale. La randare, dacă în `integrationMetadata` ai `hasGuardian: "true"`, pentru **ambele** tag-uri se folosește mai întâi apărătorul (`guardianFullName` / `guardianAddress`), apoi elevul dacă lipsește; dacă `hasGuardian` e fals sau lipsește, se folosește mai întâi elevul, apoi apărătorul. Ordinea `|` din DOCX nu contează pentru aceste perechi. Alte tag-uri cu `|` rămân „prima valoare nevidă” stânga→dreapta. `hasGuardian` (și opțional alte chei) vin din metadata salvată la draft.
