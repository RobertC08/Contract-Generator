...
import { contractsActions, templatesActions, anafActions } from "@/lib/convex-actions";
...
export const Route = createFileRoute("/_app/_auth/panou/_layout/contracte/$contractId/editeaza")({
  component: ContracteEditeazaPage,
});

function ContracteEditeazaPage() {
  const { contractId } = Route.useParams();
  const id = contractId as Id<"contracts">;

  const editData = useQuery(
    api.contracts.getForEdit,
    id ? { contractId: id } : "skip"
  );
  ...
  const loadError = id && editData === null ? "Contract negăsit sau nu poate fi editat" : null;
  const templateId = editData?.templateId ?? null;

  useEffect(() => {
    if (!editData) return;
    setSignerFullName(editData.signerFullName ?? "");
    setSignerEmail(editData.signerEmail ?? "");
    setSignerRole((editData.signerRole ?? "student") as "student" | "teacher" | "guardian" | "school_music");
    setVariables(
      editData.variablesList?.length
        ? Object.fromEntries(editData.variablesList.map((p) => [p.key, p.value]))
        : {}
    );
    setParentContractId(editData.parentContractId ?? null);
  }, [editData]);

  useEffect(() => {
    if (!editData?.templateId) return;
    let cancelled = false;
    getTemplateContentHtml({ templateId: editData.templateId })
      .then((res) => {
        if (cancelled) return;
        const defs = (res.variableDefinitions ?? []) as Array<{ name: string }>;
        const content = res.content;
        const names = content ? extractVariables(content) : defs.map((d) => d.name);
        setVariableDefinitions((res.variableDefinitions ?? null) as VariableDefinitions | null);
        setVarNames(names);
        setTemplateContent(
      ...
      .catch(() => {
        if (!cancelled) {
          setTemplateContent("<!DOCTYPE html><html><head><meta charset=\"utf-8\"></head><body><p class=\"text-zinc-500\">Previzualizare indisponibilă.</p></body></html>");
          setVarNames(
            editData?.variablesList?.length ? editData.variablesList.map((p) => p.key) : []
          );
        }
      });
    return () => { cancelled = true; };
  }, [editData?.templateId, getTemplateContentHtml]);
  ...
  if (!editData || !templateContent || !templateId) {
    return (
      <main className="w-full max-w-6xl mx-auto px-4 sm:px-6 py-4 sm:py-6">
    ...
    <main className="w-full max-w-6xl mx-auto px-4 sm:px-6 py-4 sm:py-6">
      {parentContractId && (
        <Link
          ...
          params={{ contractId: parentContractId }}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 w-full lg:max-w-sm">
          ...
          {status === "success" && signingLinks && signingLinks.length > 0 && (
            <div className="rounded-lg bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 p-3 space-y-2">
              ...
              <Link to="/panou/audit" search={{ contractId: id }} className="mt-2 inline-block text-sm text-green-700 dark:text-green-300 hover:underline">
