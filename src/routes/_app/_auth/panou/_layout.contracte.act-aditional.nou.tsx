...
const actAditionalNouSearchSchema = {
  parentContractId: (v: unknown): Id<"contracts"> | undefined =>
    typeof v === "string" && v.trim() ? (v.trim() as Id<"contracts">) : undefined,
};

export const Route = createFileRoute("/_app/_auth/panou/_layout/contracte/act-aditional/nou")({
  validateSearch: (search: Record<string, unknown>) => ({
    parentContractId: actAditionalNouSearchSchema.parentContractId(search.parentContractId),
  }),
  component: ActAditionalNouPage,
});

function ActAditionalNouPage() {
  const { parentContractId } = Route.useSearch();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const result = await createTemplate({
        ...
        addendumForContractId: parentContractId,
      });
  ...
  if (!parentContractId) {
    return (
      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-4 sm:py-6">
        ...
        <Link to="/panou/sabloane" className="mt-4 inline-block text-sm text-zinc-900 dark:text-zinc-100 underline">← Template-uri</Link>
      </main>
    );
  }
    ...
    return (
      <main className="max-w-xl mx-auto px-4 sm:px-6 py-4 sm:py-6">
        ...
        <Link
          ...
          search={{ templateId: savedTemplateId, parentContractId }}
