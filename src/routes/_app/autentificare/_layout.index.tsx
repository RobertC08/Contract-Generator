...
function AutentificarePage() {
  ...
  useEffect(() => {
    ...
    if (!isLoading && isAuthenticated && !user.username) {
      navigate({ to: "/bun-venit" });
      return;
    }
    if (!isLoading && isAuthenticated) {
      navigate({ to: redirect || "/panou" });
      return;
    }
  }, [user, isLoading, isAuthenticated, navigate, redirect]);
...
function LoginForm({ onSubmit }: { onSubmit: (email: string) => void }) {
  ...
  return (
    <div className="mx-auto flex h-full w-full max-w-96 flex-col items-center justify-center gap-6 px-6 py-12">
      <div className="mb-2 flex flex-col gap-2">
        <h3 className="text-center text-2xl font-medium text-primary">
          Autentificare - Contract Generator
        </h3>
        <p className="text-center text-base text-primary/60">
          Bine ai revenit! Introdu email-ul pentru a continua.
        </p>
      </div>
      <form
      ...
      >
        <div className="flex w-full flex-col gap-1.5">
          ...
          <form.Field
            name="email"
            validators={{
              onSubmit: z.string().max(256).email("Adresa de email nu este validă."),
            }}
            children={(field) => (
              <Input
                placeholder="Email"
        ...
        <Button type="submit" className="w-full">
          {isSubmitting ? <Loader2 className="animate-spin" /> : "Continuă cu email"}
        </Button>
      </form>
      ...
      <div className="flex w-full flex-col gap-2">
        <Button
          ...
          onClick={() => signIn("google", { redirectTo: "/autentificare" })}
        >
        ...
        <Button
          variant="outline"
          className="w-full gap-2 bg-transparent"
          onClick={() => signIn("github", { redirectTo: "/autentificare" })}
        >
          <svg
...
function VerifyForm({ email }: { email: string }) {
  ...
  return (
    <div className="mx-auto flex h-full w-full max-w-96 flex-col items-center justify-center gap-6 px-6 py-12">
      <div className="mb-2 flex flex-col gap-2">
        <p className="text-center text-2xl text-primary">Verifică inbox-ul!</p>
        <p className="text-center text-base text-primary/60">
          Ți-am trimis un cod temporar pe email.
          <br />
      ...
      >
        <div className="flex w-full flex-col gap-1.5">
          ...
          <form.Field
            name="code"
            validators={{
              onSubmit: z.string().min(8, "Codul trebuie să aibă cel puțin 8 caractere."),
            }}
            children={(field) => (
              <Input
                placeholder="Cod"
